...
function motionDots_BigPatch(subj_number, run_number)
%e.g motionDots(1,5); %subject 1 and run 5;
...
Params.General.Env_radius              = 4;      % eg, radius of whole stimuli;
Params.General.Stimuli_ecc             = [6 0];  %[horizantol(deg),vertical(deg)];eccentricity of the stimuli

Params.General.Dots_density            = 1;    % number of dots in per deg^2
...
try
    ...
    Params.General.MvLength_stimuli
    Params.General.PixStep
...
%         end
        ...
        for trial=1:trials_run
            ...
            switch temp_data(trial,2)
                case 1 % left
                    screen_patch=screen_rect_middle+[-Params.General.H_ecc_stim 0 -Params.General.H_ecc_stim 0];
                case 2 % right
                    screen_patch=screen_rect_middle+[Params.General.H_ecc_stim 0 Params.General.H_ecc_stim 0];
            end
            ...
            %------------------------make the wide image patch for movie----------------------------
            % make a wide image patch with dots.
            bigPatch_X = ceil( bps + Params.General.PixStep * Params.General.MvLength_stimuli);
            xy = diag([bigPatch_X, bps]) * rand( 2, round( Params.General.N_total_dots * bigPatch_X/bps ) );
            
            %luminance = 2*mod( randperm( size(xy,2) ), 2 ) - 1 + 126;
            ...
            %load the big matrix from screen
            big_im = Screen('GetImage', w, [0, 0, bigPatch_X, bps], 'backBuffer', [], 3);
            ...
            for frame = 1:Params.General.MvLength_stimuli
                ...
                Screen('FillOval', w,Params.General.Dot_color,[sr_hor-15, sr_ver-15, sr_hor+15, sr_ver+nn]);
                vbl=Screen('Flip',w);
